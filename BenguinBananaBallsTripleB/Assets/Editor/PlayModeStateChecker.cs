...
namespace Editor.Editor
{
    ...
    public class PlayModeStateChecker
    {
        ...
        {
            EditorApplication.playModeStateChanged += OnPlayModeStateChanged; // Subscribe to the play mode state changed event.
        }
        ...
        {
            return (from rootGameObject in GetAllRootGameObjectsInAllOpenScenes() from component in rootGameObject
                .GetComponentsInChildren<Component>(true) let serializedObject = new SerializedObject(component) from field in component
                .GetType().GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                .Where(f => f.GetCustomAttributes(typeof(Required), true).Length > 0) let property = serializedObject
                .FindProperty(field.Name) where property is { propertyType: SerializedPropertyType.ObjectReference } && property.objectReferenceValue == null select $"GameObject: '{ObjectNames.NicifyVariableName(rootGameObject.name)}' | Component: '{ObjectNames.NicifyVariableName(component.GetType().Name)}' | Field: '{ObjectNames.NicifyVariableName(field.Name)}'").ToList();
        }
        ...
        private static void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            string errorMessage = "Some required objects are missing! Please assign them before entering play mode.\n\n";
